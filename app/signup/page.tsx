...
export default function SignUp() {
  ...
  async function submit(e: FormEvent) {
    ...
    if (!terms) return setError('Please agree to the Terms & Conditions and Privacy Policy.')
  ...
  return (
    <main style={{ minHeight: '100vh', display: 'grid', placeItems: 'center', padding: 20, background: 'var(--cream)' }}>
      <div style={{ width: 'min(480px,100%)', background: 'white', padding: 38, borderRadius: 18, border: '1px solid var(--line)' }}>
        ...
        <p style={{ color: 'var(--muted)', lineHeight: 1.6 }}>Join Havenly to buy, sell, save homes and manage your marketplace activity.</p>

        {success ? (
          <div style={{ marginTop: 25, padding: 16, borderRadius: 10, background: '#eef4ef', lineHeight: 1.6 }}>
            Your account has been created. Check <strong>{email}</strong> for a confirmation link. After confirming, you will choose whether you are a buyer or seller.
            <div style={{ marginTop: 16 }}><Link href="/signin" className="btn btn-dark">Continue to sign in</Link></div>
          </div>
        ) : (
...
const labelStyle = { display: 'block', fontSize: 12, fontWeight: 700, marginTop: 18 }
