...
export default function EditListingPage() {
  ...
  useEffect(() => {
    ...
    ;(async () => {
      ...
      const { data, error } = await supabase
        .from('listings')
        .select('id,title,price,description,city,state,zip_code,seller_id')
        .eq('id', id)
  ...
  async function save(e: FormEvent) {
    ...
    if (!title || !city || !Number.isFinite(price) || price < 0) {
      setError('Please provide a valid title, price and city.')
      setSaving(false)
      return
    }
    ...
    else setMessage('Changes saved and submitted for admin review.')
    setSaving(false)
  }
    ...
    <section className="container sellPage">
      ...
      <form onSubmit={save} className="listingForm">
        <div className="formGrid">
          ...
          <label>Price (USD)<input required type="number" min="0" step="0.01" value={form.price} onChange={e=>setForm({...form,price:e.target.value})}/></label>
        ...
        <p className="muted">For safety, edited listings return to Pending Review before becoming public again.</p>
