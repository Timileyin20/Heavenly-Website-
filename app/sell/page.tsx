...
export default function SellPage() {
  ...
  return <main>
    ...
    <section className="container sellPage">
      ...
      {message && <div className="successBox">✓ {message} <a href="/">View Havenly</a></div>}
      ...
      <form onSubmit={publish} className="listingForm">
        ...
        <div className="formGrid">
          <label className="full">Title<input value={form.title} onChange={e=>update('title',e.target.value)} placeholder={kind==='property'?'Modern 3-bedroom family home':'Modern oak dining set'} /></label>
        ...
        <div className="photoNote">Your images are stored in your Supabase <b>listing-images</b> bucket. We only send the photos you select.</div>
        <button disabled={loading || !user} className="btn btn-dark publishBtn">{loading ? 'Publishing…' : 'Publish listing →'}</button>
      </form>
    </section>
  </main>
}
