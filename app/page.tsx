...
useEffect(() => {
  ...
  {h.property_mode === 'rent' ? `$${Number(h.price).toLocaleString()}/mo` : `$${Number(h.price).toLocaleString()}`}
      ...
      <section className="section container" id="marketplace">
        ...
        <div className="itemGrid">{items.map(i => <article className="itemCard" key={i.id}><div className="itemImage"><img src={i.images?.[0] || 'https://images.unsplash.com/photo-1567538096630-e0c55bd6374c?auto=format&fit=crop&w=900&q=85'} alt={i.title}/><button className="heart">♡</button></div><div className="itemBody"><div><h3>{i.name}</h3><p>{i.condition}</p></div><strong>{i.price}</strong></div></article>)}</div>
      </section>
